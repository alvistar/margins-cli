#!/usr/bin/env node
import { r as __toESM, t as __commonJSMin } from "./chunk-BbwQpWto.mjs";
...
const digits = /* @__PURE__ */ new Set("0123456789");
...
function getProperty(object, path, value) {
	if (!isObject(object) || typeof path !== "string") return value === void 0 ? object : value;
	...
	*/
	...
	get store() {
		try {
			...
			if (this.#options.clearInvalidConfig && error.name === "SyntaxError") return createPlainObject();
			throw error;
		}
	}
	set store(value) {
...
function getStore() {
	if (!_store) {
		...
		if (!cwd) {
			const xdgBase = process.env["XDG_CONFIG_HOME"] || path$1.join(os$1.homedir(), ".config");
			const xdgConfig = path$1.join(xdgBase, "margins", "config.json");
			if (fs$1.existsSync(xdgConfig)) cwd = path$1.dirname(xdgConfig);
		}
		_store = new Conf({
			projectName: "margins",
			projectSuffix: "",
...
function readLocalConfig() {
	...
	while (true) {
		const candidate = path$1.join(dir, ".margins.json");
		if (fs$1.existsSync(candidate)) {
			const raw = fs$1.readFileSync(candidate, "utf-8");
			try {
