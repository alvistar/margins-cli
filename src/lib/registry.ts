...
export function writeRegistry(registry: RepoRegistry): void {
  const regPath = registryPath()
 ...
 */
export function addRepo(registry: RepoRegistry, entry: RepoEntry): boolean {
  const normalizedNew = normalize(entry.path)
  if (registry.repos.some(r => normalize(r.path) === normalizedNew)) {
    return false
  }
